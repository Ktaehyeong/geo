...
사전에 등록된 질문을 ChatGPT 등 생성형 AI에 입력한 뒤, AI의 답변과 Citation URL을 수집하여 출처 도메인·브랜드·유형별 인용 현황을 확인할 수 있습니다. 수집된 결과는 질문별로 누적되며, 최종 데이터는 Excel 형태로 다운로드하여 AI 답변에서 어떤 브랜드와 정보 출처가 주로 인용되는지 분석하는 데 활용할 수 있습니다. 이를 통해 AI가 빈도 높게 인용하는 페이지의 유사성을 기준으로 우리의 콘텐츠를 제작하는 것을 주요 목적으로 합니다.

**사용 방법:** 질문 확인 → 생성형 AI에 질문 입력 → 답변 및 Citation 복사 → 본 페이지에 붙여넣기 → Citation 분석 및 저장 → 전체 질문 완료 후 Excel 다운로드 → 유사성 검토
""")

st.caption(
    "※ Citation 결과와 현재 질문 위치는 자동 저장됩니다. 창을 닫았다 다시 열어도 마지막 체크포인트를 우선 복구합니다."<br>
    "※ 질문 생성은 자유로우나 기본 질문은 리스닝마인드의 CEP파인더 기준으로, 해외여행/가족여행/패키지여행/여름휴가라는 키워드 기준의 예상 질문을 사용하였습니다." 
